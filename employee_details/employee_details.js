const employees = [
  ...
  function findEmployeeBySpecialization(employeeSpecialization){
    ...
    const displayArea=document.getElementById('employeeDetails');
    if(foundEmployee){
        displayArea.innerHTML=`<p>${foundEmployee.id}: ${foundEmployee.name} - ${foundEmployee.department} - $${foundEmployee.salary} - ${foundEmployee.specialization}</p>`
    } else{
        displayArea.innerHTML= `No employee found with the Specialization Java`;
    }
